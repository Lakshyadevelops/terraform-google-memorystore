...
resource "google_project_service_identity" "memorystore_sa" {
...
resource "time_sleep" "wait_for_memorystore_sa_ready_state" {
  create_duration = "30s"
  depends_on      = [google_project_service_identity.memorystore_sa]
}
